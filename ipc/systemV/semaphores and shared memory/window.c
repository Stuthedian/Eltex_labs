...
void* wait_for_message(void* arg_struct)
{
	...
	while(1)
	{
		...
		decrement_sem(args->client_semaphore_id);
		if(args->message->type == NEW_USER)
			add_new_user(args->wnd, args->message->username);
		else if(args->message->type == NEW_MESSAGE)
			add_new_message(args->wnd, args->message->text);
	}
}
